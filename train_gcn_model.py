...
class SignLanguageGCNModel(nn.Module):
    ...
    def __init__(self, in_features: int = 3, num_nodes: int = 21, 
                 hidden_dim: int = 128, num_classes: int = 26):
...
def train_gcn_model(npz_file: str, epochs: int = 100, batch_size: int = 64, 
                    learning_rate: float = 0.001, num_classes: int = 26) -> None:
    ...
    model = SignLanguageGCNModel(num_classes=num_classes).to(device)
...
if __name__ == "__main__":
    ...
    candidate_paths = [
        "asl_alphabet_landmarks.npz",  # Complete A-Z dataset
        "landmarks_dataset.npz",       # Old 24-class dataset
    ...
    if npz_path:
        ...
        # Dynamically determine the number of classes from the labels inside the NPZ
        data = np.load(npz_path, allow_pickle=True)
        unique_labels = np.unique(data['labels'])
        num_classes = len(unique_labels)
        print(f"Detected {num_classes} unique classes in the dataset (Labels: {unique_labels})")
        
        train_gcn_model(npz_path, epochs=100, batch_size=64, learning_rate=0.001, num_classes=num_classes)
    else:
        print("Error: Dataset not found in any of the expected paths:")
        for path in candidate_paths:
            print(f"  - {path}")
